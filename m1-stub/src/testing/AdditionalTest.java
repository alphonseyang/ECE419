...
public class AdditionalTest extends TestCase {
    ...
    public void testMultipleClientDifferentKey() {
        ...
        for (i = 0; i < numGetClients; i++) {
            MutlipleClients[i] = new ClientWrapper("localhost", 50000, "" + i, 50, ClientWrapper.TestType.PutGetDifferentKey);
    ...
    public void testMultipleClientUpdateSameKey() {
        ...
        for (i = 0; i < ClientNumbers; i++) {
            MutlipleClients[i] = new ClientWrapper("localhost", 50000, "" + i, 50, ClientWrapper.TestType.PutSameKey);
            threads[i] = new Thread(MutlipleClients[i]);
            threads[i].start();
        }
        for (i = 0; i < threads.length; i++) {
        ...
        for (i = 0; i < ClientNumbers; i++) {
            MutlipleClients[i] = new ClientWrapper("localhost", 50000, "" + i, 50, ClientWrapper.TestType.UpdateGetSameKey);
            threads[i] = new Thread(MutlipleClients[i]);
            threads[i].start();
        }
        for (i = 0; i < threads.length; i++) {
    ...
    public void testLRUcache() {
        kvClient_LRU = new KVStore("localhost", 50010);
    ...
    public void testLFUcache() {
        kvClient_LFU = new KVStore("localhost", 50020);
