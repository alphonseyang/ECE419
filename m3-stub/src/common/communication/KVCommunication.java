...
public class KVCommunication implements Runnable {
    ...
    public void run() {
        try {
            if (kvServer != null) {
                while (isOpen) {
                    try {
                        ...
                        if (intentionalClose && coordinatorHost != null) {
                            logger.debug("Communication channel closed, coordinator: " + coordinatorPort + " -> replica: " + kvServer.getPort());
                        } else if (intentionalClose && coordinatorHost == null) {
                            logger.debug("Communication channel closed for client or transferring data");
                        } else if (coordinatorHost != null) {
                            try {
                                closeCommunication();
                            } catch (IOException ioe2){
                                logger.error("Communication channel close error " + ioe2);
                            }
                            kvServer.storageServiceRecovery(coordinatorHost, coordinatorPort);
                            logger.error("Coordinator crashed, restarting coordinator: " + coordinatorHost + ":" + coordinatorPort + ioe);
                        } else {
            ...
            try {
                logger.debug("Close communicator channel in server " + kvServer.getPort());
                closeCommunication();
            } catch (IOException ioe) {
    ...
    private KVMessage processKVMessage(KVMessage message) {
        ...
        try {
            if (message != null) {
                ...
                if (!kvServer.isOldVersion() && kvServer.getServerState() == KVServer.ServerStateType.STOPPED && CommunicaionType == null) {
                    ...
                    switch (CommunicaionType) {
                        ...
                        case ECS_ConnectReplica:
                            coordinatorHost = value;
                            coordinatorPort = Integer.parseInt(key);
                            result = new KVMessageClass(StatusType.GET, key, value, CommunicationType.ECS_ConnectReplicaFinished);
                            break;
                        case ECS_ReplicaUpdate:
                            if (value != null)
                                kvServer.putKV(key, value);
                            else
                                kvServer.deleteKV(key);
                            result = new KVMessageClass(StatusType.PUT_SUCCESS, key, value, CommunicationType.ECS_ReplicaUpdateFinished);
                            break;
                        case ECS_ServerClose:
                            result = new KVMessageClass(StatusType.GET, "closed", "closed", CommunicationType.ECS_ServerClose);
                            intentionalClose = true;
                            break;
                        case ECS_GetReplica:
                            //Find replica port & IP and parse
                            //key or value
                            String replicaInfo = kvServer.getReplica();
                            result = new KVMessageClass(StatusType.GET, "Replica", replicaInfo, CommunicaionType.ECS_GetReplica);
                            break;

                        case ECS_InitWatch:
                            if(kvServer.getKV(value)!=null) {
                                kvServer.add_watch(value, key);
                                result=new KVMessageClass(StatusType.GET,"Test","Test",CommunicaionType.ECS_GetNotification);

                            }
                            else
                                result=new KVMessageClass(StatusType.GET,"Test","Test",CommunicaionType.ECS_Watch_Failure);

                            break;

                        case ECS_GetNotification:
                            String Identifer=key;
                            String test=kvServer.get_notify_message(Identifer);
                            result=new KVMessageClass(StatusType.GET,"Test",test,CommunicaionType.ECS_GetNotification);
                            break;
                        case ECS_Default:
                            break;
                    ...
                    if (!kvServer.isOldVersion() &&
                            (status == StatusType.GET && !isGetResponsibleServer(key) ||
                                    (status == StatusType.PUT && !isPutResponsibleServer(key)))) {
                                ...
                                if (!kvServer.isOldVersion() && kvServer.isWriterLocked()) {
                                    ...
                                    if ((key.contains(" ") && value != null) || key.equals("") || key.length() < 1 || key.length() > 20 || (value != null && value.length() > 122880)) {
                                        ...
                                        if (value != null && !value.equals("null") && !value.equals("")) {
                                            ...
                                            kvServer.putKV(key, value);
											kvServer.updateReplicaWithKV(key, value);
            ...
            result = new KVMessageClass(StatusType.SERVER_DISCONNECTED, null, null);
        }
    ...
    private boolean isPutResponsibleServer(String key) {

        return kvServer.isPutResponsibleServer(key);
    }

    private boolean isGetResponsibleServer(String key) {
        return kvServer.isGetResponsibleServer(key);
    }
