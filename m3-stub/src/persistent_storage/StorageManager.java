...
public class StorageManager  {
    ...
    * */
    ...
    public void initilizateStoreManager()
    {
        ...
        logger.debug("Initialize StorageManager");
    }
	...
	public Map LoadLookupTable(){
        ...
        try {
            ...
            in.close();
            fileIn.close();
            logger.debug("Load LookUp Table");
        } catch (IOException i) {
            i.printStackTrace();
    ...
    public synchronized Boolean SaveLookupTable(Map<String, LookUpTable> map){
        try {
            ...
            out.close();
            fileOut.close();
            logger.debug("Serialized data is saved in "+LookuptableName);
    ...
    public boolean DeleteKV(String Key){
        ...
        if(result) //Key Exist
        {
            ...
            logger.debug("Delete Key: "+Key+" from FileSystem");
        }
        else //Key does not exist
        {
            logger.debug("Could not find "+Key+" in File System");
            return result;
        }
        return false;
    }
    ...
    public StatusType PutKV(String Key, String Value){
       ...
       // StorageManager test= new StorageManager(key,value,null);
        ...
        if(a!=null){
            ...
            logger.debug("Update Key: "+Key+"Value: "+Value+" in FileSystem");
    ...
    public  String GetKV(String Key)throws Exception{
        ...
        if (a!=null){//Find the KV pair in the Mapping Table
           byte[] result= findMsgFromDisk(a.StartPoint,a.Length);
            ...
            logger.debug("Key: "+Key+ "exist "+" in FileSystem");
            return s;
        }
        else
        {
            logger.debug("Key: "+Key+ " does not exist "+" in FileSystem");
            return null;
    ...
    public String[] ParseByteArray(byte[]Bytes){
        ...
        logger.debug("Convert Byte Array to "+new String(Value).trim()+" in FileSystem");
    ...
    public  byte[] ParseMessage(String key,String Value,boolean Status)
    {
       ...
       // int pomAsInt = ByteBuffer.wrap(bytes).getInt();
        ...
        logger.debug("Convert Key: "+key+" to Byte Array in FileSystem");
    ...
    * */
    public synchronized long saveMsgToDisk(byte[] ByteFormatMsg){
        ...
        try {
            ...
            raf.close();
            logger.debug("Write Byte Array to disk");
    ...
    * */
    public synchronized byte[] findMsgFromDisk(long location,long length){
        ...
        try{
            ...
            raf.read(bytes);
            raf.close();
            logger.debug("Find Byte Array From disk");
    ...
    * */
    public synchronized boolean ModifyValidByte(long Start,long length){
        try {
           ...
           // }
            ...
            raf.write(bytes);
            raf.close();
            logger.debug("Modify Valid Byte at Location: "+Start);
    ...
    public void clearStorage(){
        if(synchronizedMap!=null) {
            ...
            //SaveLookupTable(synchronizedMap);
            logger.debug("Clear synchronizedMap");
        }
        File f = null;
        f=new File(this.DatabaseName);
        try {
            boolean result= f.delete();
            logger.debug("File "+this.DatabaseName+" deleted: "+result);
        ...
        try {
            boolean result= f.delete();
            logger.debug("File "+this.LoadLookupTable()+" deleted: "+result);
    ...
    public synchronized boolean deleteKVPairinRange(String[]hashRange) throws Exception
    {
        ...
        logger.debug("Remove Keys from look up table from "+startRange+" to"+endRange);
